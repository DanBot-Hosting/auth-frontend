...
export function useMouseLocation(): UseMouseLocation {
  const detect = useCallback((event: MouseEvent, target: HTMLElement) => {
    ...
    if (target.offsetWidth + position.x > window.innerWidth)
      position.x = window.innerWidth - target.offsetWidth;
    if (target.offsetHeight + position.y > window.innerHeight)
      position.y = window.innerHeight - target.offsetHeight;
